...
while abs(ub-lb) > epsilon
    ...
    [X,hodnost_CI_MOD,hodnost_final_CI_MOD(iter),g_CI_MOD,g_final_CI_MOD(iter),norma_CI_MOD,norm_final_CI_MOD(iter),cas_CI_MOD(iter),t_CI_MOD(iter),s_CI_MOD(iter),empirical_epsilon_CI_MOD(iter),sum_eig_final(iter),sum_eig_real(iter),t_var,S,V] = OPP_cvx_iter_mod_Linfty(C,A,B,W,k,epsilon,M,gamma(iter));
