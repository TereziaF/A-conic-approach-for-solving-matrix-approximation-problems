function[X_hat,hodnost_hat,g_hat,norma_hat,gamma,hodnost,iter,cas,hodnost_final_CI_MOD,g_final_CI_MOD,norm_final_CI_MOD,cas_CI_MOD,t_CI_MOD,s_CI_MOD,empirical_epsilon_CI_MOD,sum_eig_final,sum_eig_real] = bisection_CI_OBLIQUE_Frob(g0,g1,X0,X1,V0,V1,C,A,B,W, k,epsilon,M)
...
while abs(ub-lb) > epsilon
    ...
    [X,hodnost_CI_MOD,hodnost_final_CI_MOD(iter),g_CI_MOD,g_final_CI_MOD(iter),norma_CI_MOD,norm_final_CI_MOD(iter),cas_CI_MOD(iter),t_CI_MOD(iter),s_CI_MOD(iter),empirical_epsilon_CI_MOD(iter),sum_eig_final(iter),sum_eig_real(iter),Z,Y,V] = cvx_iter_mod_OBLIQUE_Frob(C,A,B,W,k,epsilon,M,gamma(iter));
